...
from utils.data import iCIFAR100_vit, iImageNetR, iDomainnetCIL,iStanford_cars, CUB200, eurosat
import json
...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args=None):
        ...
        if dataset_name == "cifar100_vit":
            class_name = {}
            self.class_list = self.train_dataset.classes
            for i in range(len(self.class_list)):
                class_name[i] = self.class_list[i]
            for i in self._class_order:
                self.class_name_list.append(class_name[i])
            print(self.class_name_list)

        elif dataset_name == "imagenetr":
            with open('./data/datautils/imagenet-r/class_name.json', 'r') as file:
            ...
            with open('./data/datautils/cub_200/class_name.json', 'r') as file:
                class_name = json.load(file)
            for i in self._class_order:
                self.class_name_list.append(class_name[i])
            print(self.class_name_list)
            ...
            with open('./data/datautils/eurosat/class_name.json', 'r') as file:
                class_name = json.load(file)
            for i in self._class_order:
                self.class_name_list.append(class_name[i])
            print(self.class_name_list)
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        idata.download_data()
        self.train_dataset = idata.train_dataset
        self.test_dataset = idata.test_dataset
