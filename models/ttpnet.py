...
from network.ttp_net import TTP_Net
...
class TTPNet(BaseLearner):
    def __init__(self, class_name, args):
        ...
        self._network=TTP_Net(class_name, self.args)
        self.APG_old = deepcopy(self._network.image_encoder.APG)
        self.acc=[]
    ...
    def _classifier_train(self,train_loader,test_loader,optimizer,scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                logits,features = self._network.aux_forward(inputs, sample)
                ...
                # gen_p=[]
                # x_querry = self._network.image_encoder(inputs, returnbeforepool=True, task_id=self._cur_task, sample=sample)[:,0,:]
                ...
                if self.args['bayes']:
                    if self._cur_task > 0:
                        loss = custom_regularization(self.APG_old, self._network.image_encoder.APG, self.args["batch_size"])

                out_gen=self._network(inputs,gen_p=[0,0],train=True,task_id=self._cur_task, sample=sample)
    ...
    def _eval_cnn(self, loader): 
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                out_logits=self._network(inputs,gen_p=[0,0],train=False,task_id=self._cur_task, sample=sample)
    ...
    def normal_eval_cnn(self,loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                logits = self._network(inputs, sample=False)
